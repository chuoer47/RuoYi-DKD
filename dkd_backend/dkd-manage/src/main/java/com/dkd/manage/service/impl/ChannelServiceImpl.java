...
public class ChannelServiceImpl implements IChannelService
{
    ...
    public int setChannel(ChannelConfigDto channelConfigDto){
        ...
        String innerCode = channelConfigDto.getInnerCode();
        //1. dto转po
        // todo 此方法会查询30次数据库，优化-->直接查询list集合然后处理
        List<Channel> list = channelConfigDto.getChannelList().stream().map(c -> {
            // 根据售货机编号和货道编号查询货道
            Channel channel = channelMapper.getChannelInfo(innerCode, c.getChannelCode());
            if (channel != null) {
                // 货道更新skuId
